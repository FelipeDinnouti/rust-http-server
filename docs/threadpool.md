...
Here the producer are the threads receiving HTTP requests, and the receiver is the workers that handle each request separatedly

**Graceful Shutdown:**

The goal is to close every thread safely without data loss, letting workers finish before they get killed.
